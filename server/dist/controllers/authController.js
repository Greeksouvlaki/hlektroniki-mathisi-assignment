...
const generateTokens = (userId, email, role) => {
    const accessToken = jwt.sign({ userId, email, role }, process.env['JWT_SECRET'], { expiresIn: (process.env['JWT_EXPIRES_IN'] || '7d') });
    const refreshToken = jwt.sign({ userId, email, role }, process.env['JWT_REFRESH_SECRET'], { expiresIn: (process.env['JWT_REFRESH_EXPIRES_IN'] || '30d') });
    return {
        accessToken,
        refreshToken,
...
export const register = async (req, res, next) => {
    try {
        ...
        if (error) {
            const appError = new Error(error?.details?.[0]?.message || 'Invalid input');
        ...
        if (existingUser) {
            const appError = new Error('User with this email already exists');
...
export const login = async (req, res, next) => {
    try {
        ...
        if (!user.isActive) {
            const appError = new Error('Account is deactivated');
            appError.statusCode = 401;
            appError.isOperational = true;
            throw appError;
        }
...
export const changePassword = async (req, res, next) => {
    try {
        ...
        if (!newPassword || newPassword.length < 8) {
            const appError = new Error('New password must be at least 8 characters long');
            appError.statusCode = 400;
            appError.isOperational = true;
            throw appError;
        }
