...
export class XAPIService {
   ...
   */
  static async sendStatement(statement: IXAPIStatement): Promise<boolean> {
    try {
   ...
   */
  static async sendQuizCompletionStatement(progress: IProgress, user: any): Promise<void> {
    const statement: IXAPIStatement = {
      actor: {
        objectType: 'Agent',
   ...
   */
  static async sendModuleCompletionStatement(progress: IProgress, user: any, moduleTitle: string): Promise<void> {
    const statement: IXAPIStatement = {
      actor: {
        objectType: 'Agent',
        name: `${user.firstName} ${user.lastName}`,
   ...
   */
  static async sendQuestionAnsweredStatement(
    userId: string,
    questionId: string,
    isCorrect: boolean,
    timeSpent: number,
   ...
   */
  static async sendLearningExperienceStatement(
    userId: string,
    moduleId: string,
    moduleTitle: string,
    duration: number
   ...
   */
  static async sendAdaptiveRecommendationStatement(
    userId: string,
    recommendation: any
  ): Promise<void> {
    const user = await User.findById(userId);
   ...
   */
  static async sendBatchStatements(statements: IXAPIStatement[]): Promise<boolean> {
    try {
      const response = await fetch(this.XAPI_ENDPOINT, {
        method: 'POST',
        headers: {
